...
# Устойчивость к радиочастотному полю  (Radiated immunity, RI)

Тест на устойчивость к излучаемым помехам представляет собой процедуру, обратную тесту на излучаемые помехи. На печатной плате (PCB) сигнал формируется источником, передается к нагрузке и возвращается к источнику, образуя замкнутый контур. Каждый путь передачи сигнала содержит замкнутый контур, через который при прохождении внешнего электромагнитного (ЭМ) поля индуцируется напряжение, как показано на рисунке.

![alt](images/4_rs_current.png)


![alt](images/4_rs_path.png)

## Кондуктивная помехоэмиссия (Conducted Emission, CE)
...
##  Устойчивость к кондуктивному воздействию  (Conducted immunity, CI )
