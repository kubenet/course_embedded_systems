...
> **Примечание для программистов C/C++**
>
> В Python нет оператора `switch`. Вы можете использовать оператор `if..elif..else`, чтобы сделать то же самое (а в некоторых случаях использовать [словарь](./data_structures.md#dictionary), чтобы сделать это быстро)
...
Оператор `for..in` — это еще один оператор цикла, который *перебирает* последовательность объектов, т. е. проходит через каждый элемент в последовательности. Подробнее о [sequences](./data_structures.md#sequence) мы поговорим в последующих главах. Что вам нужно знать прямо сейчас, так это то, что последовательность — это просто упорядоченный набор элементов.
